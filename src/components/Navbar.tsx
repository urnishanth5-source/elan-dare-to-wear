...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'shirts', label: 'Shirts and Pant' },
        ...
        <div className="lg:hidden bg-[#0e1118] border-b border-white/10 px-6 py-5 space-y-4 shadow-2xl">
          <div className="flex flex-col space-y-3 text-xs font-bold uppercase tracking-wider">
            <button
              ...
              className={`text-left py-1.5 ${activeTab === 'home' ? 'text-blue-400' : 'text-zinc-300'}`}
            >
              Home
            </button>
            {navItems.map((item) => (
              <button
                ...
                className={`text-left py-1.5 ${activeTab === item.id ? 'text-blue-400' : 'text-zinc-300'}`}
              >
                {item.label}
              </button>
            ))}
