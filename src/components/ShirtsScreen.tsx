...
export const ShirtsScreen: React.FC<ShirtsScreenProps> = ({
  ...
  const shirtItems = products.filter((p) => p.category === 'Shirts');
  const meta = CATEGORY_METADATA.Shirts;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-8">
      <div className="border-b border-white/[0.08] pb-6 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400">{shirtItems.length} in-store items</span>
        </div>
      ...
      {loading ? (
        <div className="text-center py-24 space-y-3">
          ...
          <p className="text-sm font-semibold text-zinc-300">Loading Shirts collection from Supabase...</p>
        </div>
      ) : shirtItems.length === 0 ? (
        <div className="text-center py-20 bg-[#12151d] rounded-2xl border border-white/[0.08] p-8 space-y-3">
          ...
          <h3 className="text-base font-bold text-white">No Shirts currently in stock</h3>
          <p className="text-xs text-zinc-400">Add items under category "Shirts" in the Admin Dashboard to show them here.</p>
        </div>
      ) : (
