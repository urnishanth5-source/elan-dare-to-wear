...
export const NewArrivalsScreen: React.FC<NewArrivalsScreenProps> = ({
  ...
  // Newest products from Supabase (already sorted by created_at desc)
  const newArrivals = products.slice(0, 12);
  const meta = CATEGORY_METADATA['new-arrivals'];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-8">
      ...
      <div className="border-b border-white/[0.08] pb-6 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400">{newArrivals.length} new arrivals</span>
        </div>
      ...
      {loading ? (
        <div className="text-center py-24 space-y-3">
          ...
          <p className="text-sm font-semibold text-zinc-300">Loading new arrivals from Supabase...</p>
        </div>
        ...
        <div className="text-center py-20 bg-[#12151d] rounded-2xl border border-white/[0.08] p-8 space-y-3">
          ...
          <p className="text-xs text-zinc-400">
            Add products via the Admin Dashboard to see new arrivals here.
          </p>
        </div>
      ) : (
